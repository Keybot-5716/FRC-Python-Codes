...
class MyRobot(wpilib.TimedRobot):
	...
	def robotInit(self):
		...
		self.lift_piston_izq = wpilib.DoubleSolenoid(0,2,3)
		self.lift_piston_der = wpilib.DoubleSolenoid(0,4,5)
	...
	def autonomousPeriodic(self):
		...
		state["timer_auto"] += 0.1

		if state["timer_auto"] < 6:
			self.drive.driveCartesian(0,0.4,0,0)
		elif state["timer_auto"] < 10:
	...
	def teleopPeriodic(self):
		...
		if state["codewide_breaker"] == False:	
			...
			# 	self.lift_piston_izq.set(1)
			# 	self.lift_piston_der.set(1)

			# 	if state["timer_piston"] > 180:
					
			# 		self.lift_piston_izq.set(0)
			# 		self.lift_piston_der.set(0)
